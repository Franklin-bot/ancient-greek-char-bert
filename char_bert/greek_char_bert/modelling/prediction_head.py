...
class CharMLMHead(BertLMHead):
    ...
    def logits_to_top_k_preds(self, logits, label_map, input_ids, top_k=1, **kwargs):
        """Converts the raw output logits into per-mask top-k predictions.
        ...
        top_k = min(top_k, probs.shape[2])
        top_scores, top_ids = torch.topk(probs, k=top_k, dim=2)
        ...
        preds = []
        for pred_ids_for_sequence, pred_scores_for_sequence, input_ids_for_sequence in zip(
            top_ids, top_scores, input_ids
        ):
            sequence_preds = []
            for pred_ids_for_token, pred_scores_for_token, input_id in zip(
                pred_ids_for_sequence, pred_scores_for_sequence, input_ids_for_sequence
            ):
                if int(input_id) != mask_index:
                    continue
                sequence_preds.append(
                    [
                        {
                            "token": label_map[int(pred_id)],
                            "score": float(pred_score),
                        }
                        for pred_id, pred_score in zip(
                            pred_ids_for_token, pred_scores_for_token
                        )
                    ]
                )
            preds.append(sequence_preds)
        return preds

    def logits_to_preds(self, logits, label_map, input_ids, top_k=1, **kwargs):
        """Converts the raw output logits into a list of top-1 predicted characters."""
        top_k_preds = self.logits_to_top_k_preds(
            logits, label_map, input_ids, top_k=top_k, **kwargs
        )
        return [
            [mask_predictions[0]["token"] for mask_predictions in sequence_predictions]
            for sequence_predictions in top_k_preds
        ]
    ...
    def formatted_preds(self, logits, label_map, samples, top_k=1, **kwargs):
        ...
        top_k_preds = self.logits_to_top_k_preds(
            logits, label_map, input_ids, top_k=top_k
        )
        preds = [
            [mask_predictions[0]["token"] for mask_predictions in sequence_predictions]
            for sequence_predictions in top_k_preds
        ...
        for (
            ...
            sample_lm_labels,
            sample_padding_mask,
        ) in zip(samples, preds, top_k_preds, input_ids, lm_label_ids, padding_mask):
            ...
            text_with_top_k_preds = self.insert_top_k_preds_into_text(
                masked_text, sample_top_k_preds
            )
            res.append(
                {
                    "task": "mlm",
                    "predictions": {
                        ...
                        "text_with_top_k_preds": text_with_top_k_preds,
                        "masked_text": masked_text,
                        "predictions": sample_preds,
                        "top_k_predictions": sample_top_k_preds,
                    },
                }
            )
        return res
